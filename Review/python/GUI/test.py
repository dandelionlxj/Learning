...
class Libop:
    ...
    def __init__(self):
        print("init")
        try:
            self.library = mysql.connector.connect(user=Libop.user, password=Libop.pwd, host=Libop.host,
                                                   database=Libop.db)
...
def booksin(text_insert):
    ...
    for line in string:
        ...
        t = LIB.insert(
            "insert into book values('{}','{}','{}','{}',{},'{}',{},{},{})".format(bookid, t, name, publisher, year,
                                                                                   writer, price, total, stock))
        if (t == 0):
...
def booksearch():
    ...
    for (BookIDs, book_types, names, publishers, years, writers, prices, totals, stocks) in curs:
        text.insert(INSERT, "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(BookIDs, book_types, names, publishers, years,
                                                                          writers, prices, totals, stocks))
    text.pack()
...
def display_rent(en_cardid):
    ...
    if ((cardid,) in curs):
        ...
        select = "select BookID,book_type,name,publisher,t,writer,price,total,stock from book natural join record where card_id='{}' and still=1 order by {}".format(
            cardid, orders)
        curs = LIB.select(select)
        text.insert(INSERT, "BookID\ttype\tname\tpublish\tyear\twriter\tprice\ttotal\tstock\n")
        for (BookID, book_type, name, publisher, t, writer, price, total, stock) in curs:
            text.insert(INSERT,
                        "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(BookID, book_type, name, publisher, t, writer,
                                                                      price, total, stock))
        text.pack()
    else:
...
def rent(BookID, cardid, brokerage):
    ...
    if (stock > (0,)):
        ...
        # date=curs.fetchall()
        date = str(date)
        showinfo('Out of stock',
                 "There are no more such book,The recent returning will be at:{}-{}-{}".format(date[16:20], date[22:23],
                                                                                               date[26:27]))
...
def back(BookID, cardid):
    ...
    curs = LIB.select(select)
    if (BookID,) in curs:
        select = "select record_id from record where BookID='{}' and card_id='{}' and still=1 order by rent_time".format(
            BookID, cardid)
